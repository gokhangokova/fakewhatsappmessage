...
import { Platform, Message, User, MessageStatus, ReplyTo, MessageReaction, Language, WhatsAppSettings, WhatsAppBackgroundType, WHATSAPP_BG_COLORS, WHATSAPP_BG_IMAGES, FontFamily, SUPPORTED_FONTS, SUPPORTED_LANGUAGES, DeviceType, GroupChatSettings, GroupParticipant, GROUP_CHAT_COLORS, GROUP_AVATAR_ILLUSTRATIONS } from '@/types'
...
export function TabbedSidebar({
  ...
  const addMessage = () => {
    // For group chat, cycle through participants
    if (groupSettings?.isGroupChat && groupSettings.participants.length > 0) {
      const participants = groupSettings.participants
      const lastMessage = messages[messages.length - 1]

      // Find the next participant (cycle through all participants)
      let nextIndex = 0
      if (lastMessage) {
        const currentIndex = participants.findIndex(p => p.id === lastMessage.userId)
      ...
      // For 1-1 chat, alternate between sender and receiver
      const lastMessage = messages[messages.length - 1]
  ...
  // Mobile: Editor as full-screen modal, Settings as bottom sheet
  if (isMobile) {
    // Editor - Full screen modal for better message editing
    if (activeTab === 'editor') {
      return (
        <Dialog open={isOpen} onOpenChange={(open) => !open && onClose?.()}>
          <DialogContent className="w-full h-full max-w-none max-h-none m-0 p-0 rounded-none border-0 data-[state=open]:slide-in-from-bottom data-[state=closed]:slide-out-to-bottom safe-area-inset" hideCloseButton={true}>
            ...
            <div className="flex items-center justify-between px-4 py-4 border-b border-gray-200 bg-[#d4f5e2] safe-area-top">
              ...
              <button
                onClick={onClose}
                className="p-2.5 hover:bg-white/50 rounded-full transition-colors active:scale-95"
              >
    ...
    return (
      <Sheet open={isOpen} onOpenChange={(open) => !open && onClose?.()}>
        ...
        >
          ...
          {/* Content with scroll */}
          <div className="flex-1 overflow-y-auto p-4 space-y-3 min-h-0 mobile-scroll safe-area-bottom">
            {/* Appearance Section */}
            <CollapsibleSection
              title={t.settings.appearance}
              icon={Palette}
            ...
            >
              ...
              <div className="space-y-2">
                ...
                <div className="flex gap-2">
                  ...
                  >
                    <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 814 1000" className="fill-current">
                      <path d="M788.1 340.9c-5.8 4.5-108.2 62.2-108.2 190.5 0 148.4 130.3 200.9 134.2 202.2-.6 3.2-20.7 71.9-68.7 141.9-42.8 61.6-87.5 123.1-155.5 123.1s-85.5-39.5-164-39.5c-76.5 0-103.7 40.8-165.9 40.8s-105.6-57-155.5-127C46.7 790.7 0 663 0 541.8c0-194.4 126.4-297.5 250.8-297.5 66.1 0 121.2 43.4 162.7 43.4 39.5 0 101.1-46 176.3-46 28.5 0 130.9 2.6 198.3 99.2zm-234-181.5c31.1-36.9 53.1-88.1 53.1-139.3 0-7.1-.6-14.3-1.9-20.1-50.6 1.9-110.8 33.7-147.1 75.8-28.5 32.4-55.1 83.6-55.1 135.5 0 7.8 1.3 15.6 1.9 18.1 3.2.6 8.4 1.3 13.6 1.3 45.4 0 102.5-30.4 135.5-71.3z"/>
                    </svg>
                    <span>iOS</span>
                  </button>
